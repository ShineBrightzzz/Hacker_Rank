 ...
 *  3. INTEGER_ARRAY h3
 */
void fillStack(vector<int>& h1,stack<int>& st1){
    for(int i=h1.size()-1;i>=0;i--){
        if(!st1.empty())
            st1.push(h1[i]+st1.top());
        else
            st1.push(h1[i]);    
    }
}
...
int equalStacks(vector<int>& h1, vector<int>& h2, vector<int>& h3) {
    ...
    while(!st1.empty() && !st2.empty() && !st3.empty()){
        int st1height = st1.top();
        int st2height = st2.top();
        int st3height = st3.top();
        ...
        if(st1height>=st2height && st1height>=st3height){
            ...
            st2.pop();
        }else if(st3height>=st2height && st3height>=st1height){
            st3.pop();
        }
    }
    return maxHeight;
